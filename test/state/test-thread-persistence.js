...
const { ROOT_DIR, STATE_DIR, THREAD_TTL_MS, workspaceIdForDir, resolveWorkspace, ALOLA_CONFIG } = require("../../src/config/env");
const { buildSessionMetadata, parseAlolaTarget, imagePathForAsic, defaultConstraintForAsic } = require("../../src/alola/session");
...
const gpuAsic = ALOLA_CONFIG.defaultAsic === "gfx123" ? "gfx124" : "gfx123";
...
describe("saveThreadsToDisk", () => {
  it("writes threads.json with serializable fields", () => {
    ...
    const alola = buildSessionMetadata(
      ...
      parseAlolaTarget([defaultNode, gpuAsic])
    );
    ...
    assert.equal(entry.alola.loginNode, defaultNode);
    assert.equal(entry.alola.asic, gpuAsic);
    assert.equal(entry.alola.image, imagePathForAsic(gpuAsic));
    assert.equal(entry.alola.constraint, defaultConstraintForAsic(gpuAsic));
...
describe("loadThreadsFromDisk", () => {
  ...
  it("restores Alola target metadata", () => {
    ...
    const alola = buildSessionMetadata(
      ...
      parseAlolaTarget([alternateNode, otherGpuAsic])
    );
    ...
    assert.equal(restored.alola.loginNode, alternateNode);
    assert.equal(restored.alola.asic, otherGpuAsic);
    assert.equal(restored.alola.timeLimit, ALOLA_CONFIG.defaultGpuTimeout);
