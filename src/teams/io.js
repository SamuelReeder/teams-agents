...
function sendLargeOutput(chatId, output, replyToId) {
  ...
  if (html.length <= MAX_CHUNK) {
    sendToTeams(chatId, `<b>${AI_PREFIX}</b><br>${html}`, replyToId, true);
    return;
  }
  ...
  for (let i = 0; i < chunks.length; i++) {
    const header = chunks.length > 1 ? `<b>${AI_PREFIX} ${i + 1}/${chunks.length}</b><br>` : `<b>${AI_PREFIX}</b><br>`;
...
function isBotMessage(msg) {
  ...
  if (!text) return true;
  if (text.startsWith(AI_PREFIX)) return true;
