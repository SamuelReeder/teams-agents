...
function buildHelpMessage(channel) {
  ...
  return `<b>Teams Agents</b><br><br>` +
    ...
    `<code>--alola</code> — keep the agent on the controller host while selecting the configured default remote-session target<br>` +
    `<code>--alola &lt;target&gt;</code> — select a configured login node or GPU target<br>` +
    ...
    `<b>Execution</b>: agents run locally in the selected workspace. Configured remote build/test/benchmark/runtime work should use durable remote sessions through <code>${escapeHtml(ALOLA_SESSION_BIN)}</code>.<br><br>` +
    `<b>Commands</b>:<br>` +
    ...
    `<br><b>Examples</b>:<br>` +
    `<code>${escapeHtml(examplePrefix)}Build and test the relevant worktree</code> — agent uses the configured remote session for build work<br>` +
    `<code>${escapeHtml(examplePrefix)}--alola &lt;target&gt; run the remote environment check</code><br>` +
    `<code>${escapeHtml(examplePrefix)}--model opus explain the project architecture</code><br>` +
