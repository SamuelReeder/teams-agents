...
function normalizeAsic(value, config = ALOLA_CONFIG) {
  const raw = String(value || config.defaultAsic || "gfx000").trim().toLowerCase();
  if (!ASIC_RE.test(raw)) {
...
function parseAlolaTarget(tokens = [], config = ALOLA_CONFIG) {
  ...
  const normalizedAsic = normalizeAsic(asic || config.defaultAsic || "gfx000", config);
  const normalizedNode = normalizeLoginNode(loginNode || config.defaultLoginNode || "01", config);
...
function imagePathForAsic(asic, config = ALOLA_CONFIG) {
  ...
  return String(config.imageTemplate || "remote_image_{asic}.sqsh")
...
function describeAlolaTarget(metadata) {
  ...
  const base = metadata.mode === "gpu"
    ? `remote GPU ${metadata.asic} via login ${metadata.loginNode}`
    : `remote login ${metadata.loginNode} (${metadata.asic})`;
...
function buildLoginStartCommand(metadata, config = ALOLA_CONFIG) {
  const session = shellQuote(metadata.tmuxSession);
  const container = shellQuote(metadata.container || config.defaultLoginContainer);
  const image = shellQuote(metadata.image || imagePathForAsic(metadata.asic, config));
  const enrootCommand = [
    `container=${container}`,
    `image=${image}`,
    'if ! enroot list 2>/dev/null | grep -Fxq -- "$container"; then enroot create -n "$container" "$image"; fi',
    "export ENROOT_ROOTFS_WRITABLE=y",
    'exec enroot start --rw --mount "$HOME:$HOME" "$container" bash --login',
  ].join("; ");
...
function buildGpuStartCommand(metadata, config = ALOLA_CONFIG) {
  const session = shellQuote(metadata.tmuxSession);
  const image = shellQuote(metadata.image || imagePathForAsic(metadata.asic, config));
  const mounts = shellQuote(config.remoteHomeMount || "/home/remote:/home/remote");
...
function buildSendKeysCommand(metadata, commandId) {
  assertCommandId(commandId);
  const script = `$HOME/.teams-agent/commands/${commandId}.sh`;
  ...
  const invocation = [
    ...
    `. "${script}"`,
    "__teams_agent_rc=$?",
    `printf '%s:%s\\n' ${shellQuote(done)} "$__teams_agent_rc"`,
  ].join("; ");
...
function buildRemoteScript(command, commandId) {
  assertCommandId(commandId);
  return [
    "# sourced by alola-session; do not execute directly unless debugging",
    "set +e",
    String(command || ""),
    "__teams_agent_rc=$?",
    "return $__teams_agent_rc 2>/dev/null || exit $__teams_agent_rc",
    "",
  ].join("\n");
}
...
function sshHostForNode(loginNode, config = ALOLA_CONFIG) {
  const node = normalizeLoginNode(loginNode, config);
  const template = String(config.sshHostTemplate || "login-{node}").trim();
  return template
    .replace(/\{node\}/g, node)
    .replace(/\{NODE\}/g, node)
...
function sshArgsForNode(loginNode, remoteCommand, config = ALOLA_CONFIG) {
  ...
  args.push(`${config.user}@${sshHostForNode(node, config)}`);
...
function buildAttachCommand(metadata, config = ALOLA_CONFIG) {
  ...
  args.push(...parseSshOptions(config.sshOptions));
  args.push(`${config.user}@${sshHostForNode(metadata.loginNode, config)}`);
