...
const ALOLA_WORK_RE = /\b(build|rebuild|compile|test|ctest|smoke|benchmark|bench|perf|gpu|remote|cmake|ninja|runtime|provider verification|verify providers?)\b/i;
...
function buildRoutingContext(threadInfo = null) {
  ...
  const lines = [
    ...
    "- Use local execution for ordinary code reading, editing, review, planning, and research.",
    `- For configured remote build/test/benchmark/runtime work, use \`${alolaCommand} run${threadFlag} -- <command>\`.`,
    `- For a configured remote GPU allocation, use \`${alolaCommand} run${threadFlag} --target <target> -- <command>\`.`,
    "- Do not claim remote verification unless you actually ran the requested command against the requested code state.",
  ];

  if (hasRepos || hasWorktrees) {
    lines.push(`- Optional workspace source roots found: ${hasRepos ? `\`${reposDir}\`` : "no repos/"}${hasRepos && hasWorktrees ? " and " : ""}${hasWorktrees ? `\`${worktreesDir}\`` : "no worktrees/"}. Do not assume these paths are visible inside remote sessions.`);
  }

  const alola = coerceAlolaMetadata(threadInfo?.alola, threadInfo || undefined);
  if (alola) {
    lines.push(`- This thread has an explicit remote target: ${describeAlolaTarget(alola)}.`);
  }
...
function buildPromptWithExecutionContext(threadInfo, prompt) {
  ...
  const lines = [
    "[Execution routing]",
    "The harness stays local to the runner by default. Use the configured remote session CLI for requested remote build/test/benchmark/runtime work.",
    `Harness working directory: ${workspace.dir}`,
    `Target: ${describeAlolaTarget(inferredTarget)}.`,
    `Use \`${ALOLA_SESSION_BIN} run${threadFlag}${targetFlag} -- <command>\` for the requested remote command.`,
    "Do not assume workspace-local repos or worktrees are visible inside remote sessions.",
    "Do not claim remote verification unless you actually ran the requested command against the requested code state.",
  ];
  if (inferredTarget.mode === "gpu") {
    lines.push("GPU allocation settings come from the configured remote environment.");
  }
  return `${lines.join("\n")}\n\n${prompt}`;
}
