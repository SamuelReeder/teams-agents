...
const HARNESS_FLAGS = {
  ...
  effort: envFlag("HARNESS_EFFORT_FLAG", "--effort"),
};

const rawHarnessDefaultModel = process.env.HARNESS_DEFAULT_MODEL;
const HARNESS_DEFAULT_MODEL =
  rawHarnessDefaultModel === undefined
    ? "openai/gpt-5.5"
    : (() => {
        const trimmed = rawHarnessDefaultModel.trim();
