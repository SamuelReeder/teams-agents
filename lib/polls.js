...
function savePollsToDisk() {
  ...
  for (const [id, poll] of polls) {
    ...
    for (const [msgId, pId] of pollResultThreads) {
      if (pId === id) resultThreadIds.push(msgId);
    }
    data.push({
      id,
      prompt: poll.prompt,
    ...
    for (const p of data) {
      if (p.resultThreadIds) {
        for (const msgId of p.resultThreadIds) {
          pollResultThreads.set(msgId, p.id);
        }
      }
      if (!p.active) {
        polls.set(p.id, { ...p, busy: false });
        continue;
      }
      polls.set(p.id, {
        ...p,
...
function createPoll(text, from, originThreadId) {
  ...
  console.log(`[Poll ${id}] Created: every ${parsed.intervalStr}, max ${poll.maxRuns} runs, model: ${poll.model || "default"}, prompt: "${poll.prompt.slice(0, 60)}"`);
  ...
  proc.on("close", (code) => {
    ...
    try {
      ...
      const sendResult = execSync(
        `python3 ${SCRIPTS_DIR}/send_chat.py --chat-id "${CHAT_ID}" -m - --html --json`,
