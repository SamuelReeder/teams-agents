...
function markdownToHtml(md) {
  // Claude --print output via SSH sometimes delivers literal \n escape sequences
  // instead of real newlines. Detect and normalize if needed.
  const literalNewlines = (md.match(/\\n/g) || []).length;
  const realNewlines = (md.match(/\n/g) || []).length;
  const normalized = literalNewlines > realNewlines ? md.replace(/\\n/g, "\n") : md;
