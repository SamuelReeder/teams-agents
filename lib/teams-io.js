...
const { CHAT_ID, SCRIPTS_DIR, REPLY_SCRIPT, ROOT_DIR } = require("./config");

marked.setOptions({ breaks: true, gfm: true });

const BOT_IDS_FILE = path.join(ROOT_DIR, "bot-message-ids.json");
const botMessageIds = new Set();

function saveBotIds() {
  const ids = [...botMessageIds].slice(-500);
  try { fs.writeFileSync(BOT_IDS_FILE, JSON.stringify(ids)); } catch {}
}

function loadBotIds() {
  try {
    const ids = JSON.parse(fs.readFileSync(BOT_IDS_FILE, "utf8"));
    for (const id of ids) botMessageIds.add(id);
    console.log(`[Bot] Loaded ${botMessageIds.size} bot message IDs`);
  } catch {}
}
...
function sendToTeams(message, replyToId) {
  try {
    let cmd;
    if (replyToId) {
      cmd = `python3 ${REPLY_SCRIPT} --chat-id "${CHAT_ID}" --reply-to "${replyToId}" -m - --html --json`;
    } else {
      cmd = `python3 ${SCRIPTS_DIR}/send_chat.py --chat-id "${CHAT_ID}" -m - --html --json`;
    }
