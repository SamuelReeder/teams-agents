  ...
  if (text.trim() === "/help") {
    sendToTeams(
      `<b>Teams Agents</b><br><br>` +
        ...
        `<code>--alola [node]</code> — run on Alola cluster (default node 03)<br>` +
        `<code>--model &lt;id&gt;</code> — set harness model (e.g. opus, haiku, claude-sonnet-4-6)<br>` +
        `<code>--effort &lt;level&gt;</code> — set effort (low, medium, high, max)<br><br>` +
        `<b>Commands</b>:<br>` +
        ...
        `<code>/polls</code> — list active polls<br><br>` +
...
function handlePollResultReply(poll, text, from, messageId, rootMessageId) {
  ...
  if (!threadInfo.model && poll.model) {
    threadInfo.model = poll.model;
  }
