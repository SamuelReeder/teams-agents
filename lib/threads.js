...
function pollChannel() {
  ...
  if (newMessages.length > 0) {
    ...
    for (const msg of newMessages.reverse()) {
      ...
      // Check if this message is a reply in an existing thread
      ...
      for (const [rootId, threadInfo] of threads) {
        ...
        if (threadMsgs.some((tm) => tm.id === msg.id)) {
          console.log(`[Poll] Thread reply from ${from} in thread ${rootId}: "${text.slice(0, 80)}"`);
          // Update lastSeen so pollThreads doesn't double-process
      ...
      if (!isThreadReply) {
        console.log(`[Poll] New thread from ${from} (id=${msg.id}): "${text.slice(0, 80)}"`);
