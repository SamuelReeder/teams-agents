...
const { CHAT_ID, MAX_CONCURRENT_AGENTS, ROOT_DIR, STATE_DIR, THREAD_TTL_MS, WORKSPACE_DIR, loadChannels } = require("./config");
const { sendToTeams, stripHtml, escapeHtml, isBotMessage, isAgentResponse, fetchMessages } = require("./teams-io");
...
function buildHelpMessage(channel) {
  ...
  const prefixNote = prefix
    ? `Prefix agent requests and thread replies with <code>${escapeHtml(prefix)}</code>. Bot commands (<code>!</code>) work with or without the prefix.<br><br>`
...
function commandTextForMessage(channel, text) {
  ...
  if (!isAgentInvocation(channel, text)) return text;

  const stripped = stripPrefix(text, channel.prefix);
  return stripped && stripped.startsWith("!") ? stripped : null;
}
...
function pollSingleChannel(channel) {
  ...
  if (newMessages.length > 0) {
    ...
    for (const msg of newMessages.reverse()) {
      ...
      // Bot commands work globally and, in prefixed channels, after the agent prefix.
...
module.exports = { pollAllChannels, getThreads, saveThreadsToDisk, loadThreadsFromDisk, gatherThreadMessages, classifyCommand, isAgentInvocation, commandTextForMessage, buildHelpMessage, listCronTasks, listWorkspaceCommands, listWorkspaceSkills };
