...
const ALOLA_CONTAINER_TIMEOUT_MS = 30 * 60 * 1000;
...
function spawnAlolaAgent(threadInfo, message, replyToId, maxConcurrent) {
  ...
  const remoteCmd = [
    `cd ~/ROCm-workspace && git pull --ff-only -q 2>/dev/null;`,
    `enroot start --rw --mount "$HOME:$HOME"`,
    `${containerName} --`,
    `bash -c 'cd ~/ROCm-workspace && claude ${claudeCmd}'`,
  ].join(" ");
  ...
  const proc = spawn(script, [node, remoteCmd], {
    cwd: WORKSPACE_DIR,
    env: { ...process.env },
