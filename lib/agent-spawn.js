...
function spawnAgent(threadInfo, message, replyToId, maxConcurrent = 3) {
  if (activeAgents >= maxConcurrent) {
    sendToTeams(threadInfo.chatId, "⏳ Too many agents running. Please wait...", replyToId);
  ...
  try {
    ...
    sendToTeams(threadInfo.chatId, `Invalid agent flags: ${err.message}`, replyToId);
    threadInfo.busy = false;
    return;
  }
  ...
  proc.on("error", (err) => {
    ...
    sendToTeams(threadInfo.chatId, `Failed to start agent: ${err.message}`, replyToId);
    processPending(threadInfo, replyToId, maxConcurrent);
  });
}
