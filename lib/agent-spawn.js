...
function buildRoutingContext() {
  ...
  const lines = [
    "## Environment\n",
    `You are running on **${os.hostname()}** (${os.platform()}, ${os.arch()}).`,
    `Home directory: \`${os.homedir()}\``,
    "",
    "Do not assume project paths are locally accessible — verify with `ls` or `test -d` before reading or writing. If a path is not available, check if you need to SSH to a remote machine.",
    "",
