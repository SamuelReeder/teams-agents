...
function buildClaudeArgs(threadInfo, prompt, flags) {
  const args = ["--print"];
...
function spawnAlolaAgent(threadInfo, message, replyToId, maxConcurrent) {
  ...
  // Build extra args for run-agent.sh
  const extraArgs = [];
  if (threadInfo.isFollowUp) extraArgs.push("--resume");
  if (flags.model) extraArgs.push("--model", flags.model);
  if (flags.effort) extraArgs.push("--effort", flags.effort);

  const remoteCmd = `cd ~/ROCm-workspace && git pull --ff-only -q 2>/dev/null; ${ALOLA_RUN_SCRIPT} ${containerName} ${threadInfo.sessionId} ${extraArgs.join(" ")}`;
