...
function alolaKeyConfigured() {
  ...
  if (isReadableFile(path.join("/run/secrets", "remote_ssh_key"))) return true;
...
function validateAlolaSecrets(errors, warnings) {
  ...
  if (!alolaKeyConfigured()) {
    pushWarning(warnings, "ALOLA_SSH_KEY is not configured; remote routing will fail until a readable key path or secret file is provided.");
    return;
  }
  let keyPath;
  try {
